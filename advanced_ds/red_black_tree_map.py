from advanced_ds.tree_map import TreeMap
...
class RedBlackTreeMap(TreeMap):
    ...
    def _rebalance_delete(self, p):
        ...
        if n == 1:                                              # deficit exists unless child is a red leaf
            c = next(self.children(p))                          # calling children iterator
    ...
    def _fix_deficit(self, z, y):
        ...
        if not self._is_red(y):                                 # y is black; will apply Case 1 or 2
            ...
            if x is not None:                                   # Case 1: y is black and has red child; do "transfer"
            ...
            else:                                               # Case 2: y is black, but no red children: recolor
                self._set_red(y)
                if self._is_red(z):
                    self._set_black(z)                          # this solves the problem
                elif not self.is_root(z):                       # recur upward
                    self._fix_deficit(self.parent(z), self.sibling(z))
        else:                                                   # Case 3: y is red; rotate node and repeat
            self._rotate(y)
            self._set_black(y)
            self._set_red(z)
            if z == self.right(y):
